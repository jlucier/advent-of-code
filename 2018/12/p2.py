...
def calc_out():
    return sum(map(lambda t: t[0] - total_pad if t[1] == '#' else 0, enumerate(curr_gen)))

total_pad = pad_gen(20)
for g in range(1, 159):
    ...
    for i in range(4, len(curr_gen)):
        ...
        curr_gen[i-2] = rules[''.join(last_gen_vals)]
    total_pad += pad_gen()

print((int(50e9) - 158) * 81 + calc_out())
